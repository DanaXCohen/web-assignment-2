...
export const Auth_controller = {
    async register(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        try {
            ...
            const newUser = new User<Partial<IUser>>({ username, email, password: hashedPassword, firstName, lastName, age });
            await newUser.save();

            res.status(201).json({ message: 'User registered successfully', newUser });
        } catch (ex) {
            res.status(500).json({ error: `Registration failed: ${ex instanceof Error ? ex.message : 'Unknown error'}` });
        }
    },
    ...
    async logout(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        try {
            const decodedToken = jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET!) as ITokenPayload;
